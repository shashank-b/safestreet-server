...
function initialize() {
    ...
    {
      var data = {lat:eventObject[i]["Lat"], lng: eventObject[i]["Long"] , name: eventObject[i]["id"].toString(), type: "Manual" , time: eventObject[i]["Info"], City:"Severity: "+ eventObject[i]["Severity"], color:"green", size:3}
    ...
    for (i in eventObject) {
        ...
        if(eventObject[i]["latitude"] != 0 && eventObject[i]["classifier_output"] == 1 && intensityBySpeed > 0.034) {
            ...
            var data = {lat:eventObject[i]["latitude"], lng: eventObject[i]["longitude"] , name: eventObject[i]["id"].toString(), type: "Automated" , time:"Detection Time: " + date.toString().substring(0,24), City:"Samples per sec: " +eventObject[i]["win_size"].toString() + "  Reporter: "+ rep, color: markerColor, size:markerSize}
